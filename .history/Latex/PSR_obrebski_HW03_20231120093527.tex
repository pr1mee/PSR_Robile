...
\begin{landscape}
    \begin{table}[]
        ...
        \begin{tabular}{r|ccc|ccc|l|l}
            ...
            Dijkstra           & \multicolumn{1}{c|}{$\rightarrow$}                         & \multicolumn{1}{c|}{$\rightarrow$}                         & \(O(n log(n)+m)\)        & $\rightarrow$                       & $\rightarrow$ & \(O(n + m)\) & with fibonacci heap                                                                                            & optimal                        \\
            ...
            Floyd-Warshall     & \multicolumn{1}{c|}{\cellcolor[HTML]{96FFFB}$\rightarrow$} & \multicolumn{1}{c|}{\cellcolor[HTML]{96FFFB}$\rightarrow$} & \(O(n^3)\)               & $\rightarrow$                       & $\rightarrow$ & \(O(n^2)  \) & \begin{tabular}[c]{@{}l@{}}not efficient for\\ big graphs;\\ this is from all to all\end{tabular}              & optimal, very interesting      \\
            RRT                & \multicolumn{1}{c|}{ $\rightarrow$ }                       & \multicolumn{1}{c|}{$\rightarrow$}                         & \(O(K \log K)\)          & $\rightarrow$                       & $\rightarrow$ & \(O(K)    \) & near optimal                                                                                                   & near optimal, very interesting \\
            \rowcolor[HTML]{96FFFB}
            RRT* (or similar)  & \multicolumn{1}{c|}{\cellcolor[HTML]{96FFFB}$\rightarrow$} & \multicolumn{1}{c|}{\cellcolor[HTML]{96FFFB}$\rightarrow$} & \(O(K \log K)\)          & $\rightarrow$                       & $\rightarrow$ & \(O(K)    \) &                                                                                                                & near optimal, very interesting \\
            Jump Point Search  & \multicolumn{1}{c|}{}                                      & \multicolumn{1}{c|}{}                                      & \(O(n log n)\)           & $\rightarrow$                       & $\rightarrow$ & \(O(n)    \) &                                                                                                                & optimal for grid-maps          \\
